...
Everything else stays the same: the palace structure (wings, rooms, halls, tunnels), AAAK compression, the MCP server with 24 tools, auto-save hooks.
...
Per-project scan exclusions live in the project's `mempalace.yaml` as `exclude_patterns` — a list of fnmatch globs matched against the project-relative path and the basename, pruning whole directories too (`--include` force-includes still win):
...
24 tools available: search (wing/room/date filters), browse, drawer CRUD (get/list/update/delete, bulk delete-by-source), find-duplicates audit, knowledge graph, agent diary.
...
**Follow-up audit (2026-07-29):** upstream `develop` re-audited through [`aa89bd8`](https://github.com/MemPalace/mempalace/commit/aa89bd8) (tip of 2026-07-28, v3.6.0, 868 commits since `de7801e`). Ported from this window: NUL-byte + lone-surrogate sanitisation on the Postgres write path so one polluted transcript can no longer abort a whole mine run (upstream [`abaf09b`](https://github.com/MemPalace/mempalace/commit/abaf09b) #1829 + [`a27129a`](https://github.com/MemPalace/mempalace/commit/a27129a) #1833 by @mvalentsev, adapted — a single recursive pre-serialization pass covers both byte classes here because this fork's `json.dumps` uses `ensure_ascii=True`), similarity clamped to `[0, 1]` so far-away cosine matches stop reporting negative values (upstream [`eef053d`](https://github.com/MemPalace/mempalace/commit/eef053d) + [`f2bed92`](https://github.com/MemPalace/mempalace/commit/f2bed92) by @bobo-xxx, extended to all five `1 - dist` sites in `searcher.py` / `layers.py` / `mcp_server.py`), half-open `[valid_from, valid_to)` as-of intervals for KG supersession with end-of-day expansion for date-only `valid_to` (upstream [`9815f0a`](https://github.com/MemPalace/mempalace/commit/9815f0a) #1913 by @ggettert, adapted to the PG TEXT columns via a SQL CASE — the `supersede()` API portion has no caller here and is not ported), and KG temporal input validation — strict `YYYY-MM-DD` ISO dates at the MCP boundary for `as_of` / `valid_from` / `ended` plus inverted-interval rejection in `add_triple` (upstream [`4d98b05`](https://github.com/MemPalace/mempalace/commit/4d98b05) + [`abe8576`](https://github.com/MemPalace/mempalace/commit/abe8576) #1164/#1167 and [`0b8c2c1`](https://github.com/MemPalace/mempalace/commit/0b8c2c1) by @arnoldwender). The remaining candidates from this window (bulk already-mined pre-fetch, authored timestamps, `exclude_patterns`, hallways/Hebbian graph dynamics, new MCP tools) are triaged in `TO-DOS.md`-style follow-ups, and the big skips (write-routing daemon — Postgres handles concurrent writers natively; Milvus/Qdrant backends; ChromaDB repair series; upstream's own pgvector backend — this fork predates it and keeps its bespoke `PalaceDB`) follow the same rationale as above.

**Open-PR ports (2026-07-29):** re-mine integrity from upstream [PR #2088](https://github.com/MemPalace/mempalace/pull/2088) by @KeilerHirsch, adapted to PostgreSQL — instead of upstream's `chunk_total` completion marker and abort-on-failed-purge, `process_file` now files each source file through one atomic `PalaceDB.replace_file_drawers` transaction (stale-row purge + all inserts commit together, so a mine killed mid-file leaves the previous state intact and a shrunk file stops serving its old chunks), `source_mtime` is captured with the read instead of a later re-stat (the PR's TOCTOU gap: an append landing in between was stamped as mined and skipped forever), and `file_already_mined` requires every drawer in the file's mining scope to carry the current mtime instead of probing one arbitrary row. The purge and freshness check are scoped by `(ingest_mode, extract_mode)` so convo extract modes don't delete or invalidate each other's drawers — the over-match class of upstream [PR #2089](https://github.com/MemPalace/mempalace/pull/2089). The convo path adopts the same machinery: `mine_convos` files each transcript through one scoped atomic replace with the mtime captured before `normalize()` reads the file (session logs are appended to while being mined), and `register_empty_file` distinguishes genuinely-empty content — which purges the scope's stale drawers in the same transaction as the sentinel — from transient `normalize()` failures, which register the sentinel without touching mined data. Entity-detector ReDoS guard from [PR #2082](https://github.com/MemPalace/mempalace/pull/2082) by @bkr499 (issue #2063): whitespace-free runs of 30+ chars (base64, minified blobs) are collapsed before the candidate regexes run — the fork's CamelCase pattern has the same nested ambiguous quantifier that pins a mine at 100% CPU for hours, confirmed hanging on a 48-char adversarial token. Harness-chrome noise stripping: a `strip_noise` pass in `normalize()` (this fork never carried upstream's base layer) reconciling [PR #1909](https://github.com/MemPalace/mempalace/pull/1909) by @KeilerHirsch (full 10-tag slash-command envelope + ANSI CSI/OSC escapes, issue #1333; [PR #1958](https://github.com/MemPalace/mempalace/pull/1958) by @jrzmurray covers the same ground) and [PR #2064](https://github.com/MemPalace/mempalace/pull/2064) by @mazurd-acre (indent-tolerant anchors, multi-paragraph tag bodies that halt at the next same-tag opening) — `system-reminder`/`task-notification` blocks, slash-command chrome, and terminal escapes no longer land in drawers when hooks mine Claude Code transcripts. Content-hash dedup from [PR #2050](https://github.com/MemPalace/mempalace/pull/2050) by @rohanprichard, adapted to PG: `mine_convos` hashes each normalized transcript, stamps `file_content_hash` in drawer metadata (expression-indexed), and skips a file whose content another file already carries — a re-exported conversation gets a registry sentinel instead of duplicating every drawer under fresh slot IDs; pre-hash legacy drawers never match, so dedup applies to newly mined files only (existing databases must create `idx_drawers_file_hash` by hand, see `init_schema.sql`). Search-surface ports: round-trippable `drawer_id` on every `mempalace_search` hit ([PR #2090](https://github.com/MemPalace/mempalace/pull/2090) by @fatkobra — this fork's hits previously carried no ID at all), a `since`/`before` date window on `mempalace_search` implemented as SQL bounds on `filed_at` with strict `YYYY-MM-DD` validation at the MCP boundary ([PR #2000](https://github.com/MemPalace/mempalace/pull/2000) by @mvalentsev), and a read-only `mempalace_find_duplicates` audit tool — union-find clusters over a bounded pgvector cosine self-join of the `max_drawers` most recent rows in scope ([PR #1951](https://github.com/MemPalace/mempalace/pull/1951) by @et1975, reworked for PG: one SQL self-join instead of per-drawer backend probes). Skipped from the P3 window: [PR #2047](https://github.com/MemPalace/mempalace/pull/2047) (the fork's MCP handler already surfaces exception type + message, so invalid diary wings produce a readable error) and [PR #2039](https://github.com/MemPalace/mempalace/pull/2039) (no `sanitize_name` validator exists in this fork to widen — same rationale as PR #683). Bulk already-mined pre-fetch (deferred candidate from the audit window): both miners now snapshot `(source_file, mtime)` for the whole scan and ask `PalaceDB.files_already_mined` once — a VALUES-join `bool_and` per file, same strict scoped rule as the per-file check — instead of one round-trip per file, which dominated scan time on already-mined projects. `exclude_patterns` (deferred candidate from the audit window): per-project fnmatch globs in `mempalace.yaml`, applied at scan with directory pruning; force-includes keep precedence. Authored timestamps from upstream [`cff43ad`](https://github.com/MemPalace/mempalace/commit/cff43ad) (#1890) by @JosefAschauer: `mine_convos` stamps `authored_at` (max per-line ISO `timestamp` of the JSONL transcript) in drawer metadata and search hits surface it with a `filed_at` fallback, so a bulk re-mine no longer collapses every drawer's chronology to ingest time — the upstream hybrid-rank tie-break and ChromaDB backfill script have no counterpart here (no hybrid ranker; legacy drawers pick the field up as their transcripts re-mine). Drawer CRUD batch for upstream tool parity (tools 21–24): `mempalace_get_drawer`, `mempalace_list_drawers` (pagination + wing/room filters, date window as SQL bounds instead of upstream's client-side post-filter), `mempalace_update_drawer` (replace or `append:` per open [PR #1762](https://github.com/MemPalace/mempalace/pull/1762) by @fkberthold; content changes re-embed), and `mempalace_delete_by_source` (bulk cleanup for upstream issue #1722's eval-dump contamination, dry-run by default with a blast-radius preview) — upstream's `memories_filed_away` is N/A here, it reads `hooks_cli` checkpoint state this fork's bash hooks never write.
