...
**Sync status:** this fork's `main` rebases a single squash commit on top of upstream `main` at [`71736a3`](https://github.com/milla-jovovich/mempalace/commit/71736a3) (PR #142, packaging cleanup). Audited against upstream through [`3489a09`](https://github.com/milla-jovovich/mempalace/commit/3489a09) (PR #135, ChromaDB singleton cache) — nothing to port: the client-caching gain is already achieved in this fork by the module-level `get_db()` singleton in `db.py`, and the companion test fixes touch upstream-only test files (`test_mcp_server.py`, `test_searcher.py`) that don't exist here. All portable fixes merged into upstream since the fork point are absorbed: security hardening (shell injection, path traversal, hook security, error sanitisation), bounded queries, `.gitignore`-aware mining, MCP integer coercion, Windows Unicode fix, AAAK honest stats, and the expanded upstream test suite. SQLite/ChromaDB-specific upstream fixes (WAL mode, batched unbounded reads, `chromadb` version pin, ChromaDB client cache) are intentionally skipped — they don't apply to the PostgreSQL backend.
