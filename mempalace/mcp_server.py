    ...
    try:
        ...
        if results["ids"] and results["ids"][0]:
            for i, drawer_id in enumerate(results["ids"][0]):
                ...
                # Cosine distance spans [0, 2]; clamp so far-away results
                # report similarity 0 instead of a negative number.
...
# MCP protocol versions this server knows how to speak, newest first.
# Claude Code negotiates the latest supported version on the initialize
# handshake — hardcoding a single version causes newer clients to refuse
# the connection. Ported from upstream 950d52b (PR #324), by
